...
6. **Concierge/Security 要件**: 個人情報をログに残さない＋"確定/予約"前に human-in-the-loop 確認（Day 4）。
...
| Security | PII配慮＋human-in-the-loop | TODO（Concierge必須級） |
| Eval | 実行可能性% 指標 | TODO（writeupの目玉） |
